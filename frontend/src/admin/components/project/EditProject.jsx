  ...
  const { id } = useParams(); 
   ...
   useEffect(() => {
    const fetchProject = async () => {
      try {
        const { data } = await axios.put(`/api/project/edit-project/${id}`); // Assuming your backend endpoint is '/api/auth/employee/:id'
        console.log(data);
        
        if (data.success) {
          setInputs({
            projectname: data.projectData.projectname,
            projectdescreption: data.projectData.projectdescreption,
...
// return false;
    ...
    try {
      const { data } = await axios.put(`/api/project/edit-project/${id}`,formData)
